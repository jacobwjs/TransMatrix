...
// Object representing a texture shader in Direct3D 9.
...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	#ifdef _DEBUG
		result = InitializeShader(device, hwnd, "../Engine/texture.vs", "../Engine/texture.ps");
	#else
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX viewMatrix, 
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			ReportError("Missing Shader File");
		}
    ...
    samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT; // For linear interpolation: use  D3D11_FILTER_MIN_MAG_MIP_POINT
...
void TextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, CHAR* shaderFilename)
{
	...
	// Get the length of the message.
	ReportError("Error compiling shader.");
	ReportErrorNoStamp(compileErrors);
...
bool TextureShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX viewMatrix, 
											 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
